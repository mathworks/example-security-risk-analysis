% Helper function to add the flag for cell with no link to requirement
% Insensitive to link direction.
%
% Copyright 1984-2023 The MathWorks, Inc
%
function addFlagCellNotLinkedWithReq(cell_list_handle,cell_list_message,flagtype)
    for i = 1:numel(cell_list_handle)
        ...
        if ~(checkInLink || checkOutLink)
            if ~isempty(cell_list_message) 
                ...
                msg = "Cell should link to requirement";
            end
...
function check = getLinkedDomain(links)
    ...
    for link = links
        ...
        if any(strcmpi({exts,extt},'.slreqx'))
            check = 1;
            break
        end
    end
